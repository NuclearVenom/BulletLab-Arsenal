...
| Tags | `ros`, `gazebo`, `simulation`, `robot-description`, `unitree`, `low-level-control`, `joint-controller` |
...
robot = Robot.load("go1_description")

# Load a specific variant
robot = Robot.load("go1_description/go1")
```
...
# Introduction
The ROS simulation packages for Unitree robots allow you to load robots and joint controllers in Gazebo, enabling low-level control of the robot joints, including control of torque, position, and angular velocity. Note that Gazebo simulation does not support high-level control, such as walking. In addition to simulation functions, you can also control your real robots in ROS using the [unitree_ros_to_real](https://github.com/unitreerobotics/unitree_ros_to_real) packages. For real robots, you can perform both high-level and low-level control using our ROS packages.
